import os, uuid, time, threading, re, logging, atexit
            ...
            for block in blocks:
                for line in block.get("lines", []):
                    for span in line.get("spans", []):
                        ...
                        if sm:
                            shifts.append({
                                "start_h": int(sm.group(1)), "start_m": int(sm.group(2)),
                                "end_h": int(sm.group(3)),   "end_m": int(sm.group(4)),
                                "x": bbox[0], "y": bbox[1]
                            })
            ...
            for s in shifts:
                ...
                for d in dates:
                    if d["y"] < s["y"]:
                        ...
                        if x_dist <= x_thresh:
                            y_dist = s["y"] - d["y"]
                            score = y_dist * 5 + x_dist  # vertikale Nähe stärker gewichten
                            if score < best_score:
...
def create_ics(events, title):
    ...
    for i, e in enumerate(events):
        ...
        if end_dt <= start_dt:
            end_dt += timedelta(days=1)  # Mitternacht-Überlauf
...
def convert_pdf():
    ...
    try:
        pdf = f.read()  # Größe ist über MAX_CONTENT_LENGTH begrenzt
        ...
        # saubere externe URLs via url_for + ProxyFix
        ...
        logging.exception("Unhandled error in convert_pdf")
...
@app.route("/")
def index():
    # HTML niemals cachen (zusätzlich zum after_request)
